...
angular.module('TodoApp').component('todoTasks', {
    ...
    <li class="list-item" ng-repeat="item in $ctrl.todoList | filter:$ctrl.search">
      <div class="complete-button" ng-click="$ctrl.completeTask({item: item})" ng-if="!item.completed">Finished!</div>
      ...
      <i class="material-icons x" ng-click="$ctrl.removeItem({item: item})">close</i>
      ...
      <div id="pop-up-edit" ng-if="item.toggle">
        ...
        <button type="submit" id="change-it-button" ng-click="$ctrl.editItem({item: item, taskEdit: taskEdit})">Change it!</button>
